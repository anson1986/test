...
- (CGFloat)caculateOffsetXAtIndexSection:(TYIndexSection)indexSection{
    ...
    if (!_isInfiniteLoop && !_layout.itemHorizontalCenter && indexSection.index == _numberOfItems - 1) {
        ...
        CGFloat extraOffset = self.layout.itemVerticalCenter ? layout.minimumInteritemSpacing/2 + (width - itemWidth)/2 : 0;
        offsetX = leftEdge + itemWidth*(indexSection.index + indexSection.section*_numberOfItems) - extraOffset;
